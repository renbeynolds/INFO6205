...
public class Benchmark<T> {
    ...
    private long doRun(T t, boolean warmup) {
        ...
        T t1 = this.fPre != null ? this.fPre.apply(t) : t;
    ...
    public static void main(String[] args) {
        ...
        for (int k = 0; k < 5; k++) {
            ...
            // For random initial ordering
            // for (int i = 0; i < n; i++) array[i] = random.nextInt();

            // For sorted initial ordering
            // for (int i = 0; i < n; i++) array[i] = i;

            // For reverse initial ordering
            // for (int i = 0; i < n; i++) array[i] = n - i;

            // For partially sorted initial ordering
            for (int i = 0; i < n; i++) array[i] = random.nextBoolean() ? random.nextInt() : i;


            benchmarkSort(array, "InsertionSort: " + n, new InsertionSort<>(), m);
            n = n * 2; // use the doubling method for values of n
        }
    }
